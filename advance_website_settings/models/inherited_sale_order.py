...
class SaleOrder(models.Model):
	...
	def get_show_subtotal(self):
		ir_default = self.env['ir.default'].sudo().get('advance.website.settings', 'sub_total')
		return True if ir_default == None else ir_default
	def get_minimun_cart_value(self):
		ir_default = self.env['ir.default'].sudo().get('advance.website.settings', 'minimum_order_value')
		return 1 if ir_default == None else ir_default

	@api.model
	def _get_errors(self, order):
		minimum_order_value =1 if self.env['ir.default'].sudo().get('advance.website.settings', 'minimum_order_value') == None else self.env['ir.default'].sudo().get('advance.website.settings', 'minimum_order_value')
		errors = []
